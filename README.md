...
# 6. Activate
Connect all credentials in the n8n UI
Update the data table IDs to point to your tables
Update the filter nodes with your Telegram chat ID / WhatsApp number
Activate the workflow

# Customization
# Adding a New Channel
Add a new trigger node (webhook, poll, etc.)
Add a filter node to validate incoming messages
Add a "Get row(s)" node to fetch user profile from the Init table
Add an "Edit Fields" node to normalize into user_message, system_prompt_details, and last_channel
Connect to the n8nClaw agent
Add a new output in the Switch node to route responses back to the channel
